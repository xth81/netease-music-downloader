...
# ---------------- 纯 Python AES-128-ECB（FIPS-197 标准实现，零依赖） ----------------
_AES_SBOX = (
...
def aes_ecb_encrypt(data_bytes, key):
    """AES-128-ECB 加密（无填充）。纯 Python 实现，不依赖 openssl。

    输入 data_bytes 必须是 16 字节的整数倍；key 为 16 字节（str 自动编码）。
    已通过 FIPS-197 标准测试向量验证。
    """
    if isinstance(key, str):
        key = key.encode("utf-8")
    if len(key) != 16:
        raise ValueError("AES 密钥必须为 16 字节")
    if len(data_bytes) % 16 != 0:
        raise ValueError("AES 输入必须是 16 字节的整数倍")

    w = _aes_key_expansion(list(key))
    out = bytearray()

    for offset in range(0, len(data_bytes), 16):
        block = data_bytes[offset:offset + 16]
        # state[row][col]
        ...
        def mix_columns():
            for c in range(4):
                s0 = [state[r][c] for r in range(4)]
                state[0][c] = _aes_gf_mul(2, s0[0]) ^ _aes_gf_mul(3, s0[1]) ^ s0[2] ^ s0[3]
                state[1][c] = s0[0] ^ _aes_gf_mul(2, s0[1]) ^ _aes_gf_mul(3, s0[2]) ^ s0[3]
                state[2][c] = s0[0] ^ s0[1] ^ _aes_gf_mul(2, s0[2]) ^ _aes_gf_mul(3, s0[3])
                state[3][c] = _aes_gf_mul(3, s0[0]) ^ s0[1] ^ s0[2] ^ _aes_gf_mul(2, s0[3])
